...
//		return strpos($markup, '<?xml') !== false && stripos($markup, 'xhtml') === false;
    ...
    public function import($source, $sourceCharset = null) {
        ...
        $return = array();
        if ($source instanceof DOMNode && !($source instanceof DOMDocument))
            $source = array($source);
//		if (is_array($source)) {
...
//		}
        if (is_array($source) || $source instanceof DOMDocument) {
            // dom nodes
...
//	$fake->documentCreate($this->charset);
    ...
    public function markup($nodes = null, $innerMarkup = false) {
        ...
        if (isset($nodes)) {
            $markup = '';
            if (!is_array($nodes) && !($nodes instanceof DOMDocument) )
                $nodes = array($nodes);
